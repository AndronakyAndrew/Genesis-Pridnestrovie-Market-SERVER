...
- **Фото каталога и карточки** отдаются через прокси API `GET /api/images/{key}` (стрим из приватного
  MinIO), поэтому наружу нужен только один домен — MinIO не публикуется. URL абсолютные, строятся от
  `Request.Scheme/Host` (за ngrok — уже публичные). **Остаётся:** SEO og-image и фото в Telegram-постах
  всё ещё используют presigned MinIO (`minio:9000`) — Telegram/краулер их снаружи не достанут; перевести
  и их на `/api/images` (для Telegram нужен абсолютный базовый адрес API в конфиге) — отдельный шаг.
...
- **`FirstImageUrl` и список изображений** отдают абсолютные URL на прокси API `GET /api/images/{key}`
  (`Api/Listings/ImageUrls.cs`, `Api/Controllers/ImagesController.cs`). Проекция каталога хранит ключ
  (`ThumbKey`), а `OkPageAsync` превращает его в URL — без presigned на каждую карточку. Бакет остаётся
  приватным; наружу светится только API.
