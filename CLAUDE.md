...
## Правила рабочего места модератора

Закреплено при сборке рабочего места модератора 2026-09-19.

- Контакты пользователя (email/телефон) отдаёт только журналируемая ручка
  `GET /api/moderation/users/{id}`. Реестр, досье, жалобы и журнал их не содержат,
  а email не участвует в поиске реестра — иначе он стал бы нежурналируемым
  резолвером «email → аккаунт».
- Сетевой сигнал в досье — совпадение HMAC адреса входа (`refresh_tokens.CreatedByIpHash`),
  а не сырой IP. Модератору показываются только два октета (`IpPrefix`). Полный IP,
  VPN-детект и EXIF фото в интерфейс не возвращаются: сервер их намеренно не хранит.
- Номер банковской карты не хранится и не логируется нигде: чёрный список держит
  HMAC («card:» + цифры, ключ `Security:IpHashKey`) и последние 4 цифры.
- «На доработку» — это Draft с причиной, а не Rejected: триггер доверия реагирует
  только на переход в rejected, и доработка не должна закрывать автору автопубликацию.
- Решения по объявлениям (одиночные и пакетные) идут только через `IListingDecisions`,
  запись в `moderation_logs` — только через `IModerationAudit` в той же транзакции.

## Правила по контактам продавца

Закреплено после прогона 2026-09-19 (`docs/server-security-log.md`, находки 31–36).

- Телефон и настройки мессенджеров читаются ТОЛЬКО в `GetContact`.
  `SellerContactResponse` нигде больше не конструируется.
- Раскрытие контактов авторизованным требует `EmailVerified`. Регистрация
  сама по себе ничего не подтверждает, поэтому без этого условия аккаунт
  стоит ноль, а даёт 30 контактов в час.
- Квота раскрытий считается по журналу в БД, а не только счётчиком в памяти:
  счётчик в памяти обнуляется при каждом рестарте контейнера.
- Текст объявления в публичной выдаче (карточка, SEO-мета, JSON-LD, пост
  в канале) проходит через `ListingContentRisk.RedactContacts`. Сырой текст —
  только владельцу и модератору. Иначе лимит раскрытия обходится чтением
  карточки, где ни лимита, ни журнала нет.
- Неопубликованное (`Draft`/`PendingReview`/`Rejected`) по прямой ссылке
  отдаётся только владельцу и модератору. `Sold`/`Archived` — остаются
  публичными намеренно.
- `IpHash` в журналах — псевдонимизация, а не анонимизация: хранится 90 дней
  (`JournalRetention`), дальше снимается джобом. Сырой IP не попадает ни в БД,
  ни в ключи кэша — при отсутствии ключа используется литерал `no-key`.
...
- `.env` скрипты читают парсером `scripts/load-env.sh`, а не `source`: значения
  там текст, а не код shell. `SMTP_FROM_NAME=Местная площадка Genesis` при
  `source` под `set -e` роняет скрипт целиком (так с 2026-09-07 молча падал
  ночной `backup.sh`).
- Внутри контейнера запускаем только то, что в нём заведомо есть: в образе
  MinIO нет `tar`, архив тома снимает sidecar с `--volumes-from`.
- Целостность копии проверяется содержимым (`gzip -t`, пробное восстановление),
  а не размером файла: текст ошибки Docker весил 122 байта и проходил порог.
