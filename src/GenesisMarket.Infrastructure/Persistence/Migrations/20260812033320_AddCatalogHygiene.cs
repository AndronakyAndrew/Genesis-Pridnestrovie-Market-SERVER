...
{
    /// <inheritdoc />
    public partial class AddCatalogHygiene : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.AddColumn<DateTimeOffset>(
                name: "ArchiveWarningAt",
                table: "listings",
            ...
            // Индекс под сортировку каталога по умолчанию (coalesce(BumpedAt, PublishedAt, CreatedAt) DESC)
            // и под keyset-пагинацию. Частичный: только «живые» объявления каталога.
            // Выражение — сырым SQL: EF не моделирует индексы по выражению coalesce.
            migrationBuilder.Sql("""
                CREATE INDEX "IX_listings_bump_active"
                ON listings (COALESCE("BumpedAt", "PublishedAt", "CreatedAt") DESC, "Id" DESC)
                WHERE "Status" = 'active' AND "DeletedAt" IS NULL;
                """);

            // ---- Persistent job store Quartz.NET (PostgreSQL). Схема Quartz управляется
            //      той же миграцией EF, чтобы таблицы были версионированы в репозитории.
            //      Префикс QRTZ_ (Postgres сворачивает неквотированные имена в нижний регистр). ----
            migrationBuilder.Sql(QuartzSchemaSql);
        }

        /// <inheritdoc />
        protected override void Down(MigrationBuilder migrationBuilder)
        {
            migrationBuilder.Sql(QuartzDropSql);

            migrationBuilder.Sql("DROP INDEX IF EXISTS \"IX_listings_bump_active\";");
        ...
        // Каноническая схема Quartz.NET для PostgreSQL (tables_postgres.sql), IF NOT EXISTS —
        // чтобы миграция была безопасной при повторном применении на уже подготовленной БД.
        ...
        private const string QuartzDropSql = """
