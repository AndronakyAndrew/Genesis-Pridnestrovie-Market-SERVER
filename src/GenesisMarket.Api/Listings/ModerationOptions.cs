...
public sealed class ModerationOptions
{
    ...
    /// <summary>
    /// Норматив ожидания объявления в очереди, минут. Старше — «просрочено SLA» в очереди,
    /// доля решений в пределах норматива — SLA на дашборде. 4 часа: премодерация держит
    /// объявление вне каталога, и дольше автор ждать не должен.
    /// </summary>
    public int ReviewSlaMinutes { get; set; } = 240;

    /// <summary>Норматив реакции на жалобу, минут (порог на экране жалоб).</summary>
    public int ReportSlaMinutes { get; set; } = 30;

    /// <summary>Сколько дней отклонённые объявления показываются во вкладке «Отклонённые».</summary>
    public int RejectedTabDays { get; set; } = 30;

    /// <summary>
    /// Рыночная цена для карточки модератора: окно выборки (дней) и минимальный размер
    /// выборки. Меньше минимума — медиана не показывается: по трём объявлениям «рынка» нет.
    /// </summary>
    public int MarketWindowDays { get; set; } = 180;

    public int MarketMinSample { get; set; } = 5;
}
