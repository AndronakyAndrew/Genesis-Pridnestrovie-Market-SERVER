...
прогона на проде фактически не применены.**
...
| 28 | 2026-09-06 | S7 | **В Production в белом списке CORS остались адреса разработчика:** `http://localhost:3000`, `http://localhost:5000`. Auth на Bearer-токене, не на cookie, поэтому автоматической кражи сессии не даёт, но прод-периметр расширен машинами разработки | `.env` (`CORS_ALLOWED_ORIGINS`) | 2 | 1 | 3 | открыто | |
