...
**Про `Server: cloudflare`.** Снаружи в ответе есть заголовок `Server`, хотя
приложение его не отдаёт (`AddServerHeader = false`): его дописывает edge уже
после нас, и снять его на стороне зоны нельзя. Состав нашего стека он не
раскрывает — что сайт за Cloudflare, видно и по DNS. `smoke-deploy.sh` это
учитывает: на ответах с `cf-ray` допускается значение `cloudflare` и никакое
другое, на loopback заголовка `Server` не должно быть вовсе. Если снаружи
пришло что-то вроде `Kestrel` — это регрессия, деплой не принимать.

Первый прогон смоука сразу после `docker compose up -d` падает на `/health/live`:
команда возвращает управление, как только контейнер запущен, а приложению нужно
до 20 секунд (`--start-period` в `HEALTHCHECK`). Дождитесь готовности:

```bash
until [ "$(docker inspect -f '{{.State.Health.Status}}' genesis-api)" = healthy ]; do sleep 2; done
```
