...
public class ListingRejectionTests(AuthApiFactory factory) : IClassFixture<AuthApiFactory>
{
    ...
    {
        // Почта подтверждена: восстановление из архива — это публикация, и оно
        // проходит те же ворота (Publishing:RequiredVerification = Email).
        var sellerEmail = Unique("reject-clear");
        var sellerId = await factory.SeedUserAsync(sellerEmail, Password, emailVerified: true);
        var listingId = await factory.SeedListingAsync(sellerId, ListingStatus.PendingReview);
